...
- `safe_poisson` 在 `lam >= ~9.223372006e18` 时会抛出
  `ValueError`（与 numpy 自身 `Generator.poisson`
  的上界保护一致），而不会继续采样：这是在 v0.1.1
  中一次常规、此前从未执行过的兜底检查中发现并修复的——PTRS
  提议包络可以生成比 `lam` 高出约
  `lam + O(20*sqrt(lam))` 的候选 `k` 值，一旦 `lam`
  接近 `np.iinfo(np.int64).max`，输出数组的 `int64`
  转换就会静默溢出，返回看似正常但错误的样本，且不抛出任何异常。
  v0.1.0 并未对此进行防护；如果你曾在如此接近该量级的
  `lam` 上使用过本工具，请升级。
